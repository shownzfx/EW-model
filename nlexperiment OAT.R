...
sample(chooseStrategy,nrow(paramsOAT),replace = T)

paramsOAT<-  nl_param_oat(
  n=25,
...
paramsOAT<-paramsOAT %>% mutate(chooseStrategy=sample(chooseStrategy,nrow(paramsOAT),replace = T))




experimentEW <- nl_experiment(
...
resultEW<-nl_run(experimentEW,parallel = T,print_progress = T)
resultEW_backup <- resultEW
write.csv(resultsEW,"nlexperiment results.csv")
