...
experiment <- nl_experiment(
  ...
  param_values = list(
    ...
    org_budget = seq(1000,2000,250),
    buffer = seq(0,0.01,0.001)
  ),
  mapping = c(
...
ggplot(results1,aes(x=factor(buffer),y=mean_infra))+
  geom_boxplot()
...
ggplot(results2,aes(x=factor(intensity_threshold),y=mean_damage))+
  geom_boxplot()


ggplot(results2,aes(x=step_id, y=mean_damage,color=factor(buffer)))+
  geom_line()+
  facet_grid(.~buffer)
